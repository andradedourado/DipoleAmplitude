...
# ----------------------------------------------------------------------------------------------------
def d2N_dAdt_bal(B, E, rs, Z): # Equation (5)

    return np.exp(- rs / scattering_length(B, E, Z)) / (4 * np.pi * rs**2)

# ----------------------------------------------------------------------------------------------------
def compute_sph_harmonics_coeffs(l, B, iE, rs, Z):

    if Z == 7:
        data = np.loadtxt(f"{SIMULATIONS_DIR}/{PARTICLES[iZ(Z)]}/{format_filename_value(B)}nG/{int(rs)}Mpc/angular_distr_{iE:02d}.dat")
    else:
...
# ----------------------------------------------------------------------------------------------------
def Phi_l_tot(l, B, Dmin, Z):

    spec = np.loadtxt(f"{RESULTS_DIR}/spec_{PARTICLES[iZ(Z)]}_EGMF_{format_filename_value(B)}nG_Dmin_{Dmin}Mpc.dat")
    spec = spec[:len(ES)]
...
# ----------------------------------------------------------------------------------------------------
def write_sph_coeffs_srcs(l, B, Dmin, Z):

    if l == 0:
        E, coeffs = Phi_0_tot(B, Dmin, Z)
        np.savetxt(f"{RESULTS_DIR}/Figures6to8/Phi_{l}_tot_{Dmin}Mpc_{PARTICLES[iZ(Z)]}_{format_filename_value(B)}nG.dat", np.column_stack((E, coeffs)), fmt = "%.15e")

    else:
        E, coeffs = Phi_l_tot(l, B, Dmin, Z)
        np.savetxt(f"{RESULTS_DIR}/Figures6to8/Phi_{l}_tot_{Dmin}Mpc_{PARTICLES[iZ(Z)]}_{format_filename_value(B)}nG.dat", np.column_stack((E, coeffs)), fmt = "%.15e")
...
if __name__ == '__main__':
    ...
    #         write_sph_coeffs_srcs(0, 1, Dmin, Z)
    #         write_sph_coeffs_srcs(1, 1, Dmin, Z)

    for B in [0.1, 0.3, 3]:
        write_sph_coeffs_srcs(0, B, 3, 7)
        # write_sph_coeffs_srcs(1, B, 3, 7)
