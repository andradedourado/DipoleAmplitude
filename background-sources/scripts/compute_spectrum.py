...
# ----------------------------------------------------------------------------------------------------
def compute_spectrum(Zs, Dmin, Dmax, has_magnetic_field, model):
    ...
    for icts, cts in enumerate(CTSS):
        for iEs, Es in enumerate(ESS):
            ...
            spec += data * w_mag(Es, cts, Zs, Dmin, Dmax, has_magnetic_field) * w_sim(Es, cts) * w_spec(Es, Zs, model)
...
# ----------------------------------------------------------------------------------------------------
def write_spectrum(Zs, Dshell, has_magnetic_field, model):
    ...
    for idist in range(len(dist_arr) - 1):
        ...
        spec.append(compute_spectrum(Zs, dist_arr[idist], dist_arr[idist + 1], has_magnetic_field, model))
    
    np.savetxt(f"{RESULTS_DIR}/{int(Dshell)}Mpc/spec_{PARTICLES[iZs(Zs)]}_{model}_{get_EGMF_label(has_magnetic_field)}.dat", np.column_stack((ES * 1e18, np.array(spec).T / (ES[:, np.newaxis] * 1e18))), fmt = "%.15e")

# ----------------------------------------------------------------------------------------------------
if __name__ == '__main__':

    for Dshell in range(1, 10):
        write_spectrum(26, Dshell, True, 'CF2023')
